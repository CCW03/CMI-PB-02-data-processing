...
plot_pvca <- function(pvcaObj, plot_title) {
  ...
  pvca_barplot <- ggplot(data    = pvca_barplot_data,
    ...
    theme(axis.text.x = element_text(angle = 45,
                                     ...
                                     hjust = 1,
                                     size  = 12)) +
    ggtitle(wrapper(plot_title, width = 6))
...
plot_pca = function(count_data, metadata, plot_title = "PRCOMP Analysis"){
  ...
  pca_plot_PC12 <- pca_res$x %>% 
    ...
    ggplot(aes(x=PC1,y=PC2)) + geom_point(aes(color=dataset),size=3, alpha = 0.75) +
...
processCellFreq = function(master_obj, BatchCorrection = TRUE, knn = FALSE){
...
###############################################################################
processOlink <- function(master_obj, BatchCorrection = TRUE, knn = FALSE){
